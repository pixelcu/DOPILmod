return function(ModName, Modver)
	...
	local pickupanim_name =
		{ PickupWalkDown = true, PickupWalkLeft = true, PickupWalkUp = true, PickupWalkRight = true }
	...
	local function GetLang()
		return IT.MenuData and IT.MenuData.Language ~= 1 and IT.LangTabl[IT.MenuData.Language] or Options.Language
	end
	...
	function ITrsl.TranslatePickedItem(player, itemID, IsTrinket)
		if itemID then
			if
				IT.MenuData.CollTranslate ~= true
				and not IsTrinket
				and IT.CollTabl[itemID]
				and CheckModEnable(IT.CollTabl[itemID]) == true
			then -- Options.Language
			...
			elseif
				not IT.MenuData.TrinkTranslate
				and IsTrinket
				and IT.TrinkTabl[itemID]
				and CheckModEnable(IT.TrinkTabl[itemID]) == true
			then
	...
	function ITrsl.TranslateBirthright(playerID)
		if IT.MenuData.CollTranslate ~= true then
			...
			if
				IT.CollTabl[CollectibleType.COLLECTIBLE_BIRTHRIGHT]
				and IT.CollTabl[CollectibleType.COLLECTIBLE_BIRTHRIGHT][GetLang()]
			then
	...
	function ITrsl.PickupCollCheck(_, player, ent, bool)
		if ent.Type == EntityType.ENTITY_PICKUP then
			...
			if
				(not PickData.PickupTouched or PickData.PickupTouched <= 0)
				and PickData.PickupIgnoreID ~= ent.Index --нагромождение, но вроде работает
				and ent.Variant == 300
				and not pickupanim_name[player:GetSprite():GetAnimation()]
			then
		...
		if player and player:ToPlayer() then
			...
			then
				if
					...
					or (
						player:GetOtherTwin()
						and player:GetOtherTwin():HasCollectible(CollectibleType.COLLECTIBLE_BIRTHRIGHT)
					)
				then
					if
						player:GetMainTwin().Index ~= player.Index
						or player:GetMainTwin().InitSeed ~= player.InitSeed
					then
						return true
					end
				end
	...
	function ITrsl.CheckHoldingCardPill()
		...
		for i = 0, game:GetNumPlayers() - 1 do
			...
			if PocketItem and slot1 == false then
				if
					IT.CollTabl[PocketItem]
					and CheckModEnable(IT.CollTabl[PocketItem])
					and IT.CollTabl[PocketItem][lang]
				then
					local str = IT.CollTabl[PocketItem][lang][1]
					if
						Input.IsActionPressed(ButtonAction.ACTION_MAP, player.ControllerIndex)
						and IT.CollTabl[PocketItem][lang][2]
					then
		...
		if game:GetFrameCount() > 10 and hud:IsVisible() and IT.MenuData.pickup_renderTranslate ~= true then
			...
			for i, k in pairs(PocketItemStrings) do
				...
				if k.Name and k.player and k.player:ToPlayer() then
					...
					if
						(pType ~= PlayerType.PLAYER_ESAU and pType ~= PlayerType.PLAYER_JACOB)
						or (
							(pType == PlayerType.PLAYER_ESAU or pType == PlayerType.PLAYER_JACOB)
							and Input.IsActionPressed(ButtonAction.ACTION_DROP, k.player.ControllerIndex)
						)
					then
					...
					if
						i > 1
						and PocketItemStrings[1]
						and PocketItemStrings[1].player
						and PocketItemStrings[1].player:GetPlayerType() == PlayerType.PLAYER_ESAU
					then
						id = id - 1
					end
					if pType == PlayerType.PLAYER_ESAU then
						local Corner = Vector(Isaac.GetScreenWidth(), Isaac.GetScreenHeight())
						local Offset = -Vector(Options.HUDOffset * 16 + 25.0, Options.HUDOffset * 6 + 47)
						...
						if id == 0 then
							if pType == PlayerType.PLAYER_JACOB then
								...
								local Offset = Vector(Options.HUDOffset * 20 + 24, Options.HUDOffset * 11 + 30)
								local Pos = Corner + Offset + shakeOffset
								font:DrawStringScaledUTF8(
									str,
									Pos.X,
						...
						elseif id == 1 then
							...
							local heartoffset = k.player:GetMaxHearts() > 18
									and 10 * math.ceil((k.player:GetMaxHearts() - 18) / 6)
								or 0

							local Corner = Vector(Isaac.GetScreenWidth(), 0) --Vector(0,0)
							local Offset = -Vector(Options.HUDOffset * 24.5 + 140.0, -Options.HUDOffset * 12 - 39)
							local Pos = Corner + Offset + ActiveItemOffset + shakeOffset
						...
						elseif id == 2 then
							...
							local heartoffset = k.player:GetMaxHearts() > 18
									and 5 * math.ceil((k.player:GetMaxHearts() - 18) / 6)
								or 0
							local Corner = Vector(0, Isaac.GetScreenHeight())
							local Offset = Vector(Options.HUDOffset * 22 + 28.0, -Options.HUDOffset * 6 - 11)
							local Pos = Corner + Offset + ActiveItemOffset + shakeOffset
						...
						elseif id == 3 then
							...
							local heartoffset = k.player:GetMaxHearts() > 18
								...
								or 0
							local Corner = Vector(Isaac.GetScreenWidth(), Isaac.GetScreenHeight())
							local Offset = -Vector(Options.HUDOffset * 16 + 149, Options.HUDOffset * 6 + 11)
							local Pos = Corner + Offset + ActiveItemOffset + shakeOffset
